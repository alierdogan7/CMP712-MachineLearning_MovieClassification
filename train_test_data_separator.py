...
def prepare_dataset(input_folder, train_output_folder, test_output_folder):
    ...
    try:
        if not path.isdir(train_output_path):
            mkdir(train_output_path, 0o755)
        if not path.isdir(test_output_path):
            mkdir(test_output_path, 0o755)
    except OSError:
        print("Directory cannot be opened in %s" % test_output_path)
